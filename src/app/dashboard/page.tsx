        ...
        <div key={i} className="flex gap-2 items-start">
          ...
          <div className="flex-1 space-y-2">
            <div className="grid grid-cols-2 gap-2">
              <AutocompleteInput value={dest.country} onChange={(v) => update(i, "country", v)} placeholder="국가" label="" searchType="country" />
              <AutocompleteInput value={dest.city} onChange={(v) => update(i, "city", v)} placeholder="도시" label="" searchType="city" extraQuery={dest.country} />
            </div>
            <div className="grid grid-cols-2 gap-2">
          ...
          {destinations.length > 1 && (
            <button type="button" onClick={() => remove(i)} className="mt-2 text-muted hover:text-red-500 flex-shrink-0"><X size={14} /></button>
          )}
        </div>
      ))}
      {destinations.length > 1 && (
...
export default function Home() {
  ...
  const daysUntil = upcoming
    ...
    : 0;
  const upcomingLabel = upcoming ? tripLabel(upcoming) : null;
  ...
  return (
    <div className="space-y-6">
      ...
      {/* 다음 여행 배너 */}
      {upcoming && upcomingLabel && daysUntil > 0 && (
        <div className="relative bg-gradient-to-r from-primary-dark via-primary to-primary-light rounded-2xl p-5 sm:p-6 text-white overflow-hidden">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-6">
            <div>
              <h2 className="text-xl font-bold">{upcomingLabel.cities}</h2>
              <p className="text-white/70 text-sm">{upcomingLabel.countries} · {upcoming.start_date} — {upcoming.end_date}</p>
            </div>
